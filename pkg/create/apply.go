...
func rewriteGitRootDockerfile(path string) error {
	...
	if strings.Contains(string(data), "composer install -d /var/www/drupal --no-interaction --no-progress --prefer-dist --no-dev --optimize-autoloader") &&
		strings.Contains(string(data), "COPY --link composer.json composer.lock /var/www/drupal/") &&
		strings.Contains(string(data), "cp /usr/share/drupal/default_settings.txt /var/www/drupal/assets/default_settings.txt") &&
